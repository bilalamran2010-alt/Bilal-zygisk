...
int SilentTarget = 0; // 0 = Head, 1 = Hip
...
float SilentChestRate = 0.0f; // For Silent 1
// Silent 1 Config
...
void AimbotRageVoid()
{
    ...
    Quaternion rot =
        GetRotationToLocation(enemyPos, 0.1f, camPos);
...
//        if (!IsValidPlayer(local_player))
//            return old_BLAGCMCGEJG1(ist, HitObject);

        auto *ClosestEnemy = GetClosestEnemyCached();
//        if (!IsSafeEnemy(ClosestEnemy))
//        return old_BLAGCMCGEJG1(ist, HitObject);

        if (ClosestEnemy != nullptr)
        {
//            if (!IsValidPlayer(ClosestEnemy))
//                return old_BLAGCMCGEJG1(ist, HitObject);
